...
function buildUrl(cfg:any){ const base=String(cfg.baseUrl||"").replace(/\/+$/,""); const ub=cfg.urlBase?("/"+String(cfg.urlBase).replace(/^\/+|\/+$/g,"")):""; const u=new URL(`${base}${ub}/sabnzbd/api`); u.searchParams.set("mode","addfile"); if(cfg.apiKey && !u.searchParams.has("apikey")) u.searchParams.set("apikey", cfg.apiKey); return u }
export async function POST(req: NextRequest){
  ...
  // Stream original multipart body through; do not set content-type explicitly (keep boundary)
  const res = await fetch(initial, withOptionalHeaders({ method:"POST", body: req.body as any, cache:"no-store", redirect:"follow" }, cfg.headers))
