...
function buildUrl(cfg:any, id:string){ const base=String(cfg.baseUrl||"").replace(/\/+$/,""); const ub=cfg.urlBase?("/"+String(cfg.urlBase).replace(/^\/+|\/+$/g,"")):""; const u=new URL(`${base}${ub}/sabnzbd/api`); u.searchParams.set("mode","queue"); u.searchParams.set("name","delete"); u.searchParams.set("value", id); if(cfg.apiKey && !u.searchParams.has("apikey")) u.searchParams.set("apikey", cfg.apiKey); return u }
export async function DELETE(_req: NextRequest, context: any){
  const id = (context as any)?.params?.nzoId
  if(!id) return NextResponse.json({ error:"Missing nzoId" }, { status:400 })
  let cfg,url:URL; try{ cfg=resolveServiceConfig("sabnzbd"); url=buildUrl(cfg, id)}catch{ return NextResponse.json({error:"SABnzbd is not configured"},{status:400}) }
  const initial = url.toString()
  const res = await fetch(initial, withOptionalHeaders({ method:"DELETE", cache:"no-store", redirect:"follow" }, cfg.headers))
  ...
  out.headers.set("x-upstream-url-initial", initial); out.headers.set("x-upstream-url-final", initial); out.headers.set("x-upstream-method","DELETE")
  return withCors(out)
}
