...
function buildUrl(cfg:any, id:string, value2:string){ const base=String(cfg.baseUrl||"").replace(/\/+$/,""); const ub=cfg.urlBase?("/"+String(cfg.urlBase).replace(/^\/+|\/+$/g,"")):""; const u=new URL(`${base}${ub}/sabnzbd/api`); u.searchParams.set("mode","queue"); u.searchParams.set("name","priority"); u.searchParams.set("value", id); u.searchParams.set("value2", value2); if(cfg.apiKey && !u.searchParams.has("apikey")) u.searchParams.set("apikey", cfg.apiKey); return u }
export async function POST(req: NextRequest, context: any){
  const id = (context as any)?.params?.nzoId
  if(!id) return NextResponse.json({ error:"Missing nzoId" }, { status:400 })
  let body:any = {}
  try { body = await req.json() } catch {}
  const val = typeof body?.priority !== "undefined" ? String(body.priority) : undefined
  if (!val) return NextResponse.json({ error:"Missing priority" }, { status:400 })
  let cfg,url:URL; try{ cfg=resolveServiceConfig("sabnzbd"); url=buildUrl(cfg, id, val)}catch{ return NextResponse.json({error:"SABnzbd is not configured"},{status:400}) }
