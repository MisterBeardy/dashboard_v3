...
export async function GET(
  ...
  try {
    ...
    cfg = resolveServiceConfig('radarr')
    primary = buildArrApiUrl(cfg, `/movie/${id}`, 'v3')
    ...
    return NextResponse.json({ error: 'Radarr is not configured' }, { status: 400 })
  }
...
export async function DELETE(
  ...
  const request = new Request(primary.toString(), headers)
  const res = await fetch(request)
