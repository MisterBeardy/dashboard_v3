...
export async function GET() {
  try {
    ...
    const cfg = resolveServiceConfig('readarr')
    
    // Create OpenAPI 3.0 specification for Readarr
    const openapiSpec = {
      openapi: '3.0.0',
      info: {
        title: 'Readarr API',
        version: '0.1.1.1371',
        description: 'API for Readarr - Ebook collection manager for Usenet and BitTorrent users',
        contact: {
          name: 'Readarr Team',
      ...
      servers: [
        {
          url: '/api/readarr',
          description: 'Dashboard Readarr API Proxy'
        }
      ],
      paths: {
        '/author': {
        ...
        '/book': {
          get: {
            ...
            description: 'Returns all books in your collection',
            tags: ['Books'],
            parameters: [
              {
                name: 'apikey',
        ...
        '/wanted/missing': {
          get: {
            summary: 'Get missing books',
            description: 'Returns all missing books',
            tags: ['Wanted'],
            parameters: [
              {
                name: 'page',
    ...
    return NextResponse.json(
      { error: 'Readarr is not configured' },
      { status: 400 }
    )
  }
}