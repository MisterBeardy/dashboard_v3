...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    primary = buildArrApiUrl(cfg, "/command", "v3")
  ...
  const used = primary
  const res = await fetch(used.toString(), withOptionalHeaders({ method: "POST", headers: { "Content-Type": "application/json" }, body: req.body, cache: "no-store", redirect: "follow" }, cfg.headers))
