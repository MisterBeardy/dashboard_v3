...
export async function GET(req: NextRequest) {
  ...
  const match = pathname.match(/\/series\/([^/]+)\b/)
  ...
  try {
    ...
    primary = buildArrApiUrl(cfg, `/series/${encodeURIComponent(id)}`, "v3")
    if (cfg.apiKey && !primary.searchParams.has("apikey")) primary.searchParams.set("apikey", cfg.apiKey)
  } catch {
    return NextResponse.json({ error: "Sonarr is not configured" }, { status: 400 })
  }

  const used = primary
  const res = await fetch(used.toString(), withOptionalHeaders({ method: "GET", cache: "no-store", redirect: "follow" }, cfg.headers))
  const initial = primary.toString(), final = used.toString()

  if (!res.ok) {
    const upstreamBody = await res.text().catch(() => "")
    return withCors(NextResponse.json({ error: "Upstream request failed", status: res.status, statusText: res.statusText, upstreamInitial: initial, upstreamFinal: final, upstreamBody }, { status: res.status }))
  }
...
export async function PUT(req: NextRequest) {
  ...
  const used = primary
  const res = await fetch(used.toString(), withOptionalHeaders({ method: "PUT", headers: { "Content-Type": "application/json" }, body: req.body, cache: "no-store", redirect: "follow" }, cfg.headers))
  const initial = primary.toString(), final = used.toString()
  ...
  out.headers.set("x-upstream-url-initial", initial)
  out.headers.set("x-upstream-url-final", final)
  out.headers.set("x-upstream-method", "PUT")
  return withCors(out)
}
